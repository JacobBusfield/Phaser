...
var Game = {	
  ...
  create: function () {
    ...
    this.ravensNo = 1;
    game.time.events.loop(Phaser.Timer.SECOND * 3, ravensUp, this);
    game.time.events.loop(Phaser.Timer.SECOND, scoreUp, this);
    ...
    this.healthText = game.add.text(50, 20, "Lives: " + 3, this.styleScore);
  ...
  update: function () 
    ...
    this.healthText.text = "Lives: " + p.health;
	
    // if player dies go back to menu screen.
		if (!p.exists)
		{
			this.state.start('Menu');
		}

    // If new raven needs spawning - spawn it.
		if (rGroup.countLiving() < this.ravensNo) 
		{
